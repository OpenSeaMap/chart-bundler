...
public class OsmcbTile
{
	...
	/**
	 * Returns the image of the tile. Creates one if necessary
	 */
	...
	public void drawSubImage(BoundingRect dest_area, BufferedImage dest_image)
	{
		...
		/*
		 * --- Iterate over all pixels of the destination image. Unfortunately we need this technique because source and dest do not have exactly the same zoom
		 * level, so the source image has to be compressed or expanded to match the destination image ---
		 */
		...
		for (int y = 0; y < maxy; y++)
		{
			...
			src_c_y = dest_area.getNorth() + (dest_area.getSouth() - dest_area.getNorth()) * y / maxy;
			...
			for (int x = 0; x < maxx; x++)
			{
				...
				src_c_x = dest_area.getWest() + (dest_area.getEast() - dest_area.getWest()) * x / maxx;

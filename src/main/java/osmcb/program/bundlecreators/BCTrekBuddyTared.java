...
public class BCTrekBuddyTared extends BCTrekBuddy
{
	...
	{
		createAtlasTarArchive(bundle.getName());
	}

	private void createAtlasTarArchive(String name)
	{
		log.trace("Creating " + name + ".tar for bundle in dir \"" + bundleDir.getPath() + "\"");

		File[] atlasLayerDirs = OSMCBUtilities.listSubDirectories(bundleDir);
		List<File> atlasMapDirs = new LinkedList<File>();
		for (File dir : atlasLayerDirs)
			OSMCBUtilities.addSubDirectories(atlasMapDirs, dir, 0);

		TarArchive ta = null;
		File crFile = new File(bundleDir, name + ".tar");
		try
		{
			ta = new TarArchive(crFile, bundleDir);

...
public class MapTileBuilder
{
	...
	private final ACBundleCreator atlasCreator;
	...
	private final IfTileImageDataWriter tileImageDataWriter;
	 ...
	 *          affects the tile size at the left and bottom border of the iMap. If <code>true</code> those tile will have the size of the remaining image data
	 *          available size (they can be smaller than the size specified). If <code>false</code> the tile size will be as specified by the iMap's
	 *          {@link TileImageParameters}.
	 */
	public MapTileBuilder(ACBundleCreator atlasCreator, MapTileWriter mapTileWriter, boolean useRealTileSize)
	{
	 ...
	 *          {@link TileImageParameters}.
	 */
	public MapTileBuilder(ACBundleCreator atlasCreator, IfTileImageDataWriter tileImageDataWriter, MapTileWriter mapTileWriter, boolean useRealTileSize)
	{
		...
		// left upper point on the iMap in pixels
		// regarding the current zoom level
		xStart = xMin * tileSize;
		yStart = yMin * tileSize;

		// lower right point on the iMap in pixels
		// regarding the current zoom level
		...
		if (useRealTileSize)
		{
			// Reduce tile size of overall iMap height/width
		...
		try
		{
			...
			while (yAbsPos < yEnd)
			{
				...
				while (xAbsPos < xEnd)
				{
					...
					BufferedImage tileImage = new BufferedImage(currentTileWidth, currentTileHeight, BufferedImage.TYPE_3BYTE_BGR);
					buf.reset();
					try
					{
	...
	/**
	 * Paints the graphics of the custom tile specified by the pixel coordinates <code>xAbsPos</code> and <code>yAbsPos</code> on the currently selected iMap &
	 * layer.
	 * 
	 * @param graphics
	 * @param xAbsPos
	...
	/**
	 * A simple local cache holding the last 10 loaded original tiles. If the custom tile size is smaller than 256x256 the efficiency of this cache is very high
	 * (~ 75% hit rate).
	 */

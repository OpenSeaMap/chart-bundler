...
/**
 * This may become someday a stand-alone implementation of a Img2Kap app
 * (20150125 nyr AH)
 * 
 * @author humbach
 *
 */
public class ImgToKap
{
	String makeFileName(String strFName)
	{
		return strFName + ".kap";
	}
	...
	public double latToY_WS84(double lat)
	{
		// double e = WGSexentrik;
		double s = Math.sin(lat * Math.PI / 180.0);

		return Math.log(Math.tan(Math.PI / 4.0 + lat * Math.PI / 360.0) * Math.pow((1.0 - Math.E * s) / (1.0 + Math.E * s), Math.E / 2.0));
	}
	...
	static double posToD(double lat0, double lon0, double lat1, double lon1)
	{
		...
		v = Math.sin((lon0 - lon1) / 2.0);
		w = Math.cos(lat0) * Math.cos(lat1) * v * v;
		x = Math.sin((lat0 - lat1) / 2.0);
		return (180.0 * 60.0 / Math.PI) * 2.0 * Math.asin(Math.sqrt(x * x + w));
	}
	 ...
	 * 
	 * 
	 * int imgtokap(int typein,char *filein, double lat0, double lon0, double lat1, double lon1, int optkap,int color,char *title, int units, char *sd,char
	 * *fileout);
	 ...
	 * 
	 * int writeimgkap(FILE *out,FIBITMAP **bitmap,int optkap, int optcolors, Color32 *palette, uint16_t widthin,uint16_t heightin, uint16_t widthout, uint16_t
	 * heightout)
	 * {
	 ...
	 * num_colors = HistGetPalette(colorskap,colors,palette,hist,num_colors,len,optcolors,(Color32
	 * *)FreeImage_GetPalette(*bitmap),FreeImage_GetColorsUsed(*bitmap));
	 * if (!num_colors)
	 * {
	 ...
	 * 
	 * 
	 * static int readkapheader(int typein,FILE *in,int typeout, FILE *out, char *date,char *title,int optcolor,int *widthout, int *heightout, double *rx, double
	 * *ry, int *depth, RGBQUAD *palette)
	 * 
	 * {
	 * char *s;
	 * int result = 0;
	 ...
	 * fprintf(stderr, "\t    different format are accepted : -1.22  1Â°10'20.123N  -1d22.123 ...\n");
